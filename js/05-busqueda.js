...
function localMatches(v){
  ...
  (window.DATA_COMISARIAS||[]).forEach(c=>{ if(normalize(c.nombre).includes(nq)) out.push({kind:'comis',c}); });
  (window.DATA_DIRECTORIO||[]).forEach(d=>{ if(d.direccion && normalize(d.nombre).includes(nq)) out.push({kind:'dir',c:d}); });
...
async function runSearch(v, forceGeo){
  ...
  if(locals.length){
    ...
    locals.forEach((o,i)=>{
      const c=o.c, col=colorFor(c.dsc);
      html+=`<div class="ritem" data-i="${i}">
        <div class="ic" style="background:${col}22;color:${col}">🛡️</div>
        <div class="tx"><div class="t1">${c.nombre}</div>
          <div class="t2">${dscName(c.dsc)}</div></div></div>`;
    });
  }
...
function analyzeAddress(r){
  ...
  if(zoneHit){
    ...
    const byName=comisMarkers.find(o=> o.data.dsc===dscnum && key && normalize(o.data.nombre).includes(key));
    if(byName) cria=byName.data;
  } else if(nearest){ dscnum=nearest.dsc; source='cercania'; }
  ...
  area.innerHTML=`
   ...
   <div class="card">
     ...
     <div class="kv"><div class="k">Comisaría jurisdiccional ${source==='cercania'?'(más cercana)':''}</div>
        <div class="v">${cria?cria.nombre:'—'}</div>

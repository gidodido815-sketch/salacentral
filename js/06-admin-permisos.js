...
function actualizarCapasVisibles() {
  ...
  // CAMARAS: solo usuarios y admin (NO público)
  if (window.LAYERS.cam && (permisos === PERMISOS.USUARIO || permisos === PERMISOS.ADMIN)) {
    // Cámaras disponibles para usuario
  } else if (window.LAYERS.cam && permisos === PERMISOS.PUBLICO) {
    // Público NO ve cámaras
    MAP.removeLayer(LAYERS.cam);
  }
...
function actualizarMenuCapas() {
  ...
  toggles.forEach(t => {
    ...
    } else if ((layer === 'casillas' || layer === 'garitas' || layer === 'dsto' || layer === 'cam') && permisos === PERMISOS.PUBLICO) {
      t.style.opacity = '0.5';
      t.style.pointerEvents = 'none';
    } else {
      t.style.opacity = '1';
